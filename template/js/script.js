...
function kboard_tree_category_parents(){
	if(kboard_current.use_tree_category){
		...
		jQuery('#kboard-tree-category-'+tree_category_index).append('<option value="">-권역을 선택해 주세요-</option>');
		...
		jQuery.each(tree_category, function(index, element){
			if(category_id === element.parent_id){
				if(check==0){
					...
					jQuery('#kboard-tree-category-'+(tree_category_index+1)).append('<option value="">-나라를 선택해 주세요-</option>');
